...
- `cv/awesome-cv.cls` — the template class. Prefer configuring via the hooks exposed in `cv.tex` (e.g. `\colorlet{awesome}{...}`, `\setbool{acvSectionColorHighlight}`) over editing the class directly. Local patches (re-apply if the class is ever updated from upstream): (1) a `%` after `\end{minipage}` in `\makecvheader` (line ~534) fixes a persistent 1.46pt overfull hbox caused by the newline between the photo and text minipages; (2) the `\faAlt` alt-text for mobile and email in `\makecvheader` is `\@mobile`/`\@email` instead of `tel:\@mobile`/`mailto:\@email`, so PDF text extraction returns the plain number/address without URI schemes (the schemes remain in the `\href` targets).
